...
Statistical hypothesis testing is based on rejecting the null hypothesis if the likelihood of the observed data under the null hypotheses is low.  We usually test each hypothesis separately, using some level of significance \alpha. If multiple hypotheses are tested, the probability of getting a significant result simply due to chance increases,  even if all of the tests are actually not significant. 
Most common is Bonferroni corretcion. If a significance threshold of \alpha is used (usually 0.05), and n separate tests are performed, then the Bonferroni adjustment considers a score significant only if the corresponding P-value is ≤\alpha/n.
